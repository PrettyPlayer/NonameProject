...
def save(time):
	...
	f.write("<h1><center>Рабочий файл - Noname.py</center></h1><h3><center>Потраченное время на проект:</center><center><pre>" + time[0][0] + "\t" + str(time[0][1]) + "\n" + time[1][0] + "\t" + str(time[1][1]) + "\n" + time[2][0] + "\t" + str(time[2][1]) + "\n" + time[3][0] + "\t" + str(round(time[3][1])) + "</pre></center></h3><h4>Про таймер я совсем забыл, часы не считал уже давно .-.</h4><h2>Управление:</h2><h3>Стрелочки вверх/вниз для навигации среди опций, пока их 3</h3><h3>Стрелочки влево/вправо меняют выбранную опцию</h3><h3>Пробел перелистывает страницы, если ноты открыты</h3><h3>Shift при зажатии позволяет клавише издавать протяжный звук</h3>")
